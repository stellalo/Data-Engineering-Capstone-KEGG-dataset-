...
with T37_data as (
    select KEGG as KEGG_temp, AVEG as Mean_temp from {{ref ('MDA_Temp_37')}}
),
XFC_data as (
    select KEGG as KEGG_XFC, AVEG as Mean_XFC from {{ref ('MDA_X_FC_Resistance')}}
),
Fluconazole_data as (
    select KEGG as KEGG_Fluconazole, AVEG as Mean_Fluconazole  from {{ref ('MDA_Fluconazole_Resistance')}}
),
NaCl_data as (
    select KEGG as KEGG_NaCl, AVEG as Mean_NaCl  from {{ref ('MDA_O610_NaCl')}}
)
